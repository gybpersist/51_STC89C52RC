...
static void DigtalTube_Single(u8 pos, u8 number_code)
{ // pos = 7 -> pos = 0000 0111
    // 在哪里显示数字 [位选]
    // P1 = 0x90 -->           10 010 000
...
static void DigtalTube_Refresh()
{
    static u8 i = 0;
    ...
    if (i == 7) //因为i++写在了判断的前面 所以i要判断是否等于8
    {
        i = 0;
        return;
    }   
...
void Int_DigtalTube_Init()
{
    ...
    //启动定时器中断
    Dri_Timer0_Init();
    //再把刷新数码管的函数交给定时器0的任务数组
    Dri_Timer0_RegisterCallBack(DigtalTube_Refresh);   
}
...
void Int_DigtalTube_Stop(){
    // 关闭SMG-EN 给P36赋值高电平 -> 关闭数码管开关
    P36 = 1;
    //把数码管从定时器任务数组中移除
    Dri_Timer0_DeregisterCallBack(DigtalTube_Refresh);
}